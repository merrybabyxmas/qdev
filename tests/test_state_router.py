...
class TestStateRouter(unittest.TestCase):
    def setUp(self):
        ...
        self.default_policy = {
            ...
            "thresholds": {"spread_max_bps": 50.0, "toxicity_max": 0.9, "microprice_drift_trigger": 0.05}
        }

    def test_hard_rules_event_shock(self):
        features = {"is_event_window": True, "spread": 0.01, "toxicity_vpin": 0.1, "mid_price": 100.0}
        state = self.detector.detect_state(features)
        self.assertEqual(state, MarketState.EVENT_SHOCK)

        action = self.router.route_execution(state, prediction=10.0, policy=self.default_policy, symbol="BTC", features=features)
    ...
    def test_stable_trend_aggressive(self):
        ...
        action = self.router.route_execution(state, prediction=1.0, policy=self.default_policy, symbol="BTC", features=features)
    ...
    def test_stable_mean_reversion_passive(self):
        ...
        action = self.router.route_execution(state, prediction=0.5, policy=self.default_policy, symbol="BTC", features=features)
